...
public class Shape : MonoBehaviour, IPointerClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    ...
    public void CreateShape(ShapeData shapeData)
    {
        ...
        {
            GameObject squareInstance = Instantiate(squareShapeImage, transform);
            _currentShape.Add(squareInstance);
        }
        foreach (var square in _currentShape)
        {
            ...
            for (var column = 0; column < shapeData.columns; column++)
            {
                if (shapeData.board[row].column[column])
                {
                    ...
                    _currentShape[currentIndexInList].GetComponent<RectTransform>().localPosition =
                        new Vector2(GetXPositonForShapeSquare(shapeData, column, moveDistance), GetYPositionForShapeSquare(shapeData, row, moveDistance));
                    currentIndexInList++;
                }
            }
        }
